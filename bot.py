...
messages = [
    ...
    "Ты достоин успеха, и я горжусь, что могу быть рядом.",
...
def start(update: Update, context: CallbackContext) -> None:
    ...
    update.message.reply_text("Привет! Я твой мотивационный бот. Я буду отправлять тебе вдохновляющие сообщения.")

# Отправка случайного сообщения
def send_motivation(context: CallbackContext) -> None:
    message = random.choice(messages)
    for user_id in user_ids:
        context.bot.send_message(chat_id=user_id, text=message)

# Функция для активации бота в нужные часы
def activate_bot(context: CallbackContext) -> None:
    scheduler = BackgroundScheduler()
    
    # Устанавливаем время, когда бот будет активен (с 8:00 до 22:00, каждые 2 часа)
    scheduler.add_job(send_motivation, IntervalTrigger(hours=2, start_date="2024-12-07 08:00:00", end_date="2024-12-07 22:00:00"))
    scheduler.start()
...
def main() -> None:
    ...
    # Планируем активацию бота в 8 утра и запуск функций
    scheduler.add_job(activate_bot, 'date', run_date="2024-12-07 08:00:00", args=[updater.bot])
    scheduler.start()
